...
local function send_xml(skt, resp)
    ...
    local bytesSent, err = blockingtcp.send(skt, tostring(#data).."\000"..data.."\000")
    if err then
...
local function debugger_loop(self, async_packet)
    ...
    while true do
        -- reads packet
        local packet = async_packet
        if packet == nil then
        	packet, err = read_packet(self.skt)
        	if( packet == nil ) then
...
local function sendConnectionString(skt, host, port, idekey)
...
local function init(host, port, idekey, secondsToWaitForConnection)
    ...
    local ok, err
    local numConnectRetries = math.max( (secondsToWaitForConnection or 3)*2, 1)
    for i=1, numConnectRetries do
        ok, err = blockingtcp.connect(skt, host, port)
   		if err then error(string.format("Cannot connect to %s:%d : %s", host, port, err)) end
	    
	    if ok then
	    	sendConnectionString(skt, host, port, idekey)
		    
		    -- Need to test the receive function to see if we have a connected socket
		    local readZero, err = blockingtcp.receive(skt, 0)
		    if err == nil then
		    	break
		    else
		    	skt:close()
			    skt = assert(socket.tcp())
			    blockingtcp.settimeout(skt, nil)
		    end
	    end
	    if i<numConnectRetries then
